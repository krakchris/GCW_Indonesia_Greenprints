...
# input is a shapely polygon of the park of interest and output is two dataframes with listed images and the most recent image for Summer and Winter with least amount of cloud cover
def get_SumWin(park_shape,UTM_EPSG_code,clouds_b = 'n', index = 0):
...
# Querying for WV03 or WV02 within the aoi
    ...
    query += " OR item_type:IDAHOImage OR item_type:DigitalGlobeProduct"   
...
# set cloud check 
    ...
    images_df2 = pd.DataFrame(columns=['id','month','year','type','cloud cover','overlap','check'])
    ...
    for r in results:
        ...
        images_df = images_df.append({'id': props['attributes']['catalogID'],'month':props['item_date'][5:7],'year':props['item_date'][0:4],'type':props['item_type'][1],'cloud cover':props['attributes']['cloudCover_int'],'overlap': fraction_overlap,'check':props['attributes']['cloudCover_int'] < 15 and int(props['item_date'][5:7]) >= 1 and int(props['item_date'][5:7]) <= 12 and fraction_overlap >.5 and len(props['attributes']['catalogID']) < 20},ignore_index=True)
...
#     Select the first image
    imagestatus ='unknown'
    iS = 0  # randint(0, min(2,len(selections)))
    iW = index  #randint(0, min(2,len(selectionw)))
    ...
    if (selections.empty | selectionw.empty):
        ...
        return error, error, error, error 
            ...
            for i in range(len(selections)):
                try: 
                    catalog_id_s = env.inputs.get('catalog_id', selections.id[i])
                    images = CatalogImage(catalog_id_s, band_type="MS", bbox=map(float, bbox.split(",")),proj=UTM_EPSG_code, pansharpen= False)
                    break
                except:
                    continue
                    
                   
            for i in range(len(selectionw)): 
                try:
                    catalog_id_w = env.inputs.get('catalog_id', selectionw.id[iW])    
                    imagew = CatalogImage(catalog_id_w, band_type="MS", bbox=map(float, bbox.split(",")),proj=UTM_EPSG_code, pansharpen= False)  
                    break
                except:
                    continue
